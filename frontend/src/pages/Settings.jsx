...
const Settings = () => {
  ...
  const getProviderLockMessage = (providerKey) => {
    const provider = providerInfo[providerKey];
    if (user?.plan === 'trial') {
      return 'Upgrade to Creator for advanced settings';
    }
    if (user?.plan === 'creator' && provider.badge === 'Creator Pro') {
      return 'Upgrade to Creator Pro for premium features';
    }
    return '';
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {user?.role === 'admin' && (
          <motion.div
            ...
            className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 mb-6 border border-white/20"
          >
            <h2 className="text-xl font-bold text-white mb-2">Generation Settings</h2>
            <p className="text-purple-200 text-sm mb-6">
              {user.plan === 'trial'
                ? 'Upgrade to Creator for advanced generation settings'
                : 'Manage your generation preferences'}
            </p>
            ...
            {user.plan === 'trial' && (
              <div className="mt-6 p-4 bg-purple-500/10 border border-purple-500/30 rounded-xl">
                <p className="text-purple-200 text-sm mb-3">
                  Trial users have automatic optimization enabled for best results. Upgrade to Creator to unlock provider selection.
                </p>
                <button
        ...
        {user.plan !== 'creator_pro' && user.billing_status !== 'lifetime' && (
          ...
          >
            ...
            <p className="text-purple-200 mb-4">
              Upgrade to Creator Pro for all premium features and advanced options
            </p>
            <button
              onClick={() => navigate('/pricing')}
